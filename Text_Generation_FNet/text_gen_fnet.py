# text_gen_fnet.py
# FNet transformer for text generation in Keras.
# Introduction
# The original transformer implementation (Vaswani et al., 2017) was
# one of the major breakthroughs in Natural Language Processing, giving
# rise to important architectures such as BERT and GPT. However, the 
# drawback of these architectures is that the self-attention mechanism
# they use is computationally expensive. The FNet architecture proposes
# to replace this self attention mechanism with a learner mechanism: a
# Fourier transformation-based linear mixer for input tokens.
# The FNet model was able to achieve 92-97% of BERT's accuracy while
# training 80% faster on GPU and almost 70% faster on TPUs. This type
# of design provides an efficient and small model size, leading to
# faster inference times.
# In this example, this architecture will be implemented and trained on
# the Cornel Movie Dialog corpus to show the applicability of this
# model to text generation.
# Source: https://keras.io/examples/nlp/text_generation_fnet/
...
#from tensorflow.keras.layers.experimental.preprocessing import TextVectorization # Use for TF 2.4
import os
import re
...
def main():
	...
	path_to_movie_conversions = os.path.join(
	...
	def preprocess_text(sentence):
		...
		sentence = tf.strings.regex_replace(
			sentence, r"([?.!,])", r" \1 "
		)
	...
	val_dataset = (
		val_dataset.cache().batch(BATCH_SIZE).prefetch(tf.data.AUTOTUNE)
	)
