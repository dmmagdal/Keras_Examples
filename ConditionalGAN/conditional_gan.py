# conditional_gan.py
# Training a GAN conditional on class labels to generate handwritten
# digits.
# Generative Adversarial Networks (GANs) let us genreate novel image
# data, video data, or audio data from a random input. Typically, the
# random input is sampled from a normal distribution, before going
# through a series of transformations that turn it into something
# plausible (image, video, audio, etc).
# However, a simple DCGAN doesn't let us control the appearence (e.g.
# class) of the samples we're generating. For instance, with a GAN that
# generates MNIST handwritten digits, a simple DCGAN wouldn't let us
# choose the class of digits we're generating. To be able to control
# what we generate, we need to condition the GAN output on a semantic
# input, such as the class of an image.
# In this example, we'll build a Conditional GAN that can generate
# MNIST handwritten digits conditioned on a given class. Such a model
# can have various useful applications:
# -> let's say you are dealing with an imbalanced image dataset, and
#	you'd like to gather more examples for the skewed class to balance
#	the dataset. Data collection can be a costly process on its own.
#	You could instead train a Conditional GAN and use it to generate
#	novel images for the class that needs balancing.
# -> since the generator learns to associate the generated samples with
#	class labels, its representations can also be used for other 
#	downstream tasks.
# Following are the references used for developing this example:
# -> Conditional Generative Adversarial Nets 
# -> Lecture on Conditional Generation from Coursera
# If you need a refresher on GANs, you can refer to the "Generative
# Adversarial Networks" section of this example
# (https://livebook.manning.com/book/deep-learning-with-python-second-
# edition/chapter-12/r-3/232).
# Source: https://keras.io/examples/generative/conditional_gan/
# Tensorflow 2.7
# Python 3.7
# Windows/MacOS/Linux
...
def main():
	...
	interpolation_noise = tf.repeat(
		interpolation_noise, repeats=num_interpolations
	)
	interpolation_noise = tf.reshape(
		interpolation_noise, (num_interpolations, latent_dim)
	)
